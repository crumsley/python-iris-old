...
class Authenticator(object):
	...
	def authenticate(self, **kwargs):
		...
		config = utils.parse_config(profile=self.profile)
		profile = config["profiles"][self.profile]
		if "token" in profile:
			if "timestamp" in profile:
				now = int(time.time())
				if (now - profile["timestamp"]) < 3550:
					self.token = profile["token"]
					self.response = {"success": True, "body": profile}
				else:
					regenerate = True
			else:
				regenerate = True
		...
		if regenerate == True:
			...
			if self.response["status"] == "success":
				...
				config["profiles"][self.profile]["token"] = self.response["token"]
				config["profiles"][self.profile]["timestamp"] = self.response["timestamp"]
	...
	def get_credentials(self, config=None):
		...
		payload = {"user": profile["username"], "password": profile["password"], "public": "false"}
